...
export const ReadAllEventsBackward: Definition = {
  key: 'ReadAllEventsBackward',
  title: 'Read All Events Backward',
  signature: 'readAllEventsBackward(data: IReadAllEvents, correlationId?: string, credentials?: Credentials): Observable<Event<IReadAllEventsCompleted>>',
  summary: 'Reads a max count number of events from the $all stream starting from a given commit and prepare position',
  informal: 'This operation will read a certain number of events from the $all stream moving toward earlier events',
  overview: (
    <View>
  ...
  details: (
    <>
      ...
      that were committed before that event, in descending order.  It will read up to the given max count of events and then complete.  The operation
  ...
  examples: [ {
    description: 'Reads events backward from the $all stream',
    code: Snippets.ReadAllEventsBackward,
  } ],
  references: [ {
