...
export const ReadStreamEventsForward: Definition = {
  key: 'ReadStreamEventsForward',
  title: 'Read Stream Events Forward',
  signature: 'readStreamEventsForward(data: IReadStreamEvents, correlationId?: string, credentials?: Credentials): Observable<Event<IReadStreamEventsCompleted>>',
  summary: 'Reads a max count number of events from a stream starting from a given event number',
  informal: 'This operation will read a certain number of events from a stream moving toward later events',
  overview: (
    <View>
  ...
  details: (
    <>
      This operation reads events from a single event stream starting at the given event number.  It will then read events that occurred after
      that event, in ascending order.  It will read up to the given max count of events and then complete.  The operation does not complete until all the
  ...
  examples: [ {
    description: 'Reads events forward from a stream',
    code: Snippets.ReadStreamEventsForward,
  } ],
  references: [ {
